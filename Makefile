...
CNI_TRANSPARENT_BUILD_DIR = $(BUILD_DIR)/cni-transparent
...
CNI_TRANSPARENT_ARCHIVE_NAME = azure-vnet-cni-transparent-$(GOOS)-$(GOARCH)-$(VERSION).$(ARCHIVE_EXT)
...
#transparent mode is linux only
ifeq ($(GOOS),linux)
	mkdir -p $(CNI_TRANSPARENT_BUILD_DIR)
	cp cni/azure-$(GOOS)-transparent.conflist $(CNI_TRANSPARENT_BUILD_DIR)/10-azure.conflist
	cp telemetry/azure-vnet-telemetry.config $(CNI_TRANSPARENT_BUILD_DIR)/azure-vnet-telemetry.config
	cp $(CNI_BUILD_DIR)/azure-vnet$(EXE_EXT) $(CNI_BUILD_DIR)/azure-vnet-ipam$(EXE_EXT) $(CNI_BUILD_DIR)/azure-vnet-telemetry$(EXE_EXT) $(CNI_TRANSPARENT_BUILD_DIR)
	chmod 0755 $(CNI_TRANSPARENT_BUILD_DIR)/azure-vnet$(EXE_EXT) $(CNI_TRANSPARENT_BUILD_DIR)/azure-vnet-ipam$(EXE_EXT)
	cd $(CNI_TRANSPARENT_BUILD_DIR) && $(ARCHIVE_CMD) $(CNI_TRANSPARENT_ARCHIVE_NAME) azure-vnet$(EXE_EXT) azure-vnet-ipam$(EXE_EXT) azure-vnet-telemetry$(EXE_EXT) 10-azure.conflist azure-vnet-telemetry.config
	chown $(BUILD_USER):$(BUILD_USER) $(CNI_TRANSPARENT_BUILD_DIR)/$(CNI_TRANSPARENT_ARCHIVE_NAME)
endif	
